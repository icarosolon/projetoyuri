...
class usuario_prato
{
    ...
    public function conectar($nome_do_prato, $host, $inf_prato)
    {
        global $pdo;
        global $msgErro;/**se não funcionar voltar aqui e remover linha**/
        try {
            $pdo = new PDO("mysql:dbname=".$nome_do_prato.";host=".$host,$inf_prato);
        } catch (PDOException $i) {
            $msgErro = $i->getMessage();
          }
    }
    
    public function cadastrar($nome_do_prato, $inf_prato)
    {
        global $pdo;
        ...
        {
            $sql = $pdo->prepare("INSERT INTO usuarios (nome_do_prato, inf_prato) VALUES (:np, :ip)");
            $sql->bindValue(":np",$nome_do_prato);
